...
{
    public class RefreshGoldGameBoostSystem : ReactiveSystem<MetaEntity>, IInitializeSystem
    {
        private readonly IStorageUIService _storage;
        private readonly IGroup<MetaEntity> _boosters;
        private List<MetaEntity> _boostersBuffer = new(4);

        public RefreshGoldGameBoostSystem(MetaContext meta, IStorageUIService storage) : base(meta)
        {
            _storage = storage;
